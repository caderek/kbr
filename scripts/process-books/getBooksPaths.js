...
function getAllFiles(location, extensions) {
  ...
  const dirs = entities.filter((entity) => entity.isDirectory()).map((entity) => entity.name)

  return [...files, ...dirs.map((dir) => getAllFiles(path.join(location, dir), extensions)).flat()]
}
...
fs.writeFileSync("public/raw-books/book-paths.json", JSON.stringify(getBooksPaths(), null, 2))
