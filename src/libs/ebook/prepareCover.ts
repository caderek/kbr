...
function drawText(
  ...
  for (const [i, chunk] of title.entries()) {
    ...
    ctx.fillText(chunk.toUpperCase(), ctx.canvas.width / 2, y, Math.floor(0.775 * ctx.canvas.width))
  }
}
...
function drawCover(ctx: CanvasRenderingContext2D, info: Info) {
  ...
  ctx.strokeRect(0.05 * WIDTH, 0.05 * WIDTH, ctx.canvas.width - 0.1 * WIDTH, ctx.canvas.height - 0.1 * WIDTH)
...
export async function prepareCover(file: File | null, info: Info, width: number = 320) {
